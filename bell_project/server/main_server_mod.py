...
def send_list(Client_Socket):
	...
	while (True):
		...
		if (times != old_profile):
			old_profile=times
			try:
				Client_Socket.sendall(json.dumps(times)+'?') #sending times in JSON format
				...
				#Biju: DBG
				print ( "".join([ShowTime, " Data send to: ", block_IP, " Profile: ", profileName]))
							#Biju: DBG
				debug ( "".join([ShowTime, " Data send to: ", block_IP, " Profile: ", profileName]))
				#debug ( block_IP )
				#debug (profileName)
							#Biju: DBG
				#debug ( "".join([block_IP, profileName]))
			except:
				print "Socket might be closed, stopping thread...."
				break
		time.sleep(60)
